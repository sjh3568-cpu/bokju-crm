...
- **적재기에도 같은 규칙**(2026-09-26, 이어서) — 앞으로는 손으로 `--snapshot`을 안 돌려도 되게. `tools/import_admission_roster.py`: 파일명이 하루치(`snapshot_date_from_name`)면 `plan_absent_discharges`가 파일에 없는 열린 명부 회차를 골라 미리보기에 명단(이름·병실·입원일·퇴원일·근거)을 보여주고, 적재 때 닫는다. 퇴원일은 `crm_discharge_sql`(상담 퇴원완료·앱 회차·미복귀 외진 출발) 값이 있으면 그것, 없으면 스냅샷 날짜에 '명부 미등재(추정)'. 부분 파일 가드 `SNAPSHOT_MIN_RATIO`(0.9). 이어지는 백필에 `snapshot=`을 넘겨 상담까지 맞추고, 회차가 '추정'으로 닫힌 것이면 상담 `discharge_reason`에도 그 사유가 따라간다(날짜는 회차와 같아 새 퇴원 행은 안 생김). 관리 화면 체크박스 '명부에 없는 사람은 퇴원 처리'(기본 켬), CLI `--no-absent-discharge`. `tests/test_roster_snapshot_absent.py`.
- **중복 회차 합치기**(2026-09-26) — 한 번의 입원이 명부 회차와 CRM 회차 두 줄로 남아 있었다(재원 190명). 두 줄이면 "CRM 회차는 퇴원일이 안 채워진다"는 문제가 계속 생겨 우회 장치를 덧붙이게 된다. `consultation_id`가 UNIQUE인 점을 이용해 **명부 회차를 남기고 CRM 회차를 흡수**한다(`tools/backfill_from_roster.merge_duplicate_episodes`, 백필마다 자동·멱등): 외진 기록 `episode_id` 재연결 → CRM 회차 삭제 → 상담 연결·CRM 전용 값(퇴원예정일·입원예정일시·대기 시작일)을 명부 회차로. 이후 `sync_admission_episode`의 `ON CONFLICT(consultation_id)`가 명부 회차를 UPDATE하고 roster_key 가드가 입·퇴원일을 지켜 새 중복이 안 생긴다. 날짜 창은 비대칭 — 명부가 CRM보다 늦은 건 30일까지 같은 입원(상담에 적힌 날은 예정일), 반대로 CRM이 더 늦으면 명부가 모르는 새 입원일 수 있어 3일까지만. 실측 190쌍 중 184쌍이 입원일까지 같았다. **한 명부 회차가 이미 다른 상담과 연결돼 있으면 두 번째 CRM 회차는 건드리지 않는다** — 이 가드 없이 재실행했다가 상담 4건(7137·7183·7215·7636)의 회차 연결을 끊어 `sync_admission_episode`로 복구했다. 적용 후 열린 CRM 회차 364→174, 재원·입퇴원 집계·외진 기록은 전부 불변. `tests/test_merge_duplicate_episodes.py`.
