...
TRANSPORT_REASON_OPTIONS=픽업(입원),외진,단순운행,혈액요청,기관방문,출장수행,식당퇴근   # (선택) 시트 F열 '요청이유' 확인 규칙과 같게. 규칙 밖 값은 시트가 거부
```
...
3. 관리과 차량이 안 나가면 **불필요 (보호자 직접)** 또는 **불필요 (사설구급차)** 를 저장 (그래야 대시보드 경고가 꺼짐).
   둘 다 시트에는 넣지 않고 CRM에만 남습니다 — 재원 명부 🚐✕ 에 마우스를 올리면 사유가 보입니다.
4. 시트 전송이 되면 브라우저가 **그 날짜 탭을 새 탭으로 엽니다**. 카드 오른쪽 위 **운행 시트 열기** 도
   날짜(탭) 칸의 날짜 탭을 찾아 엽니다 (탭이 없으면 "탭이 아직 없습니다" 안내, 시트 첫 화면으로 떨어지지 않음).
5. 저장 후 상태
