...
| `SMS_PROVIDER` / `SMS_API_KEY` / `SMS_API_USER` / `SMS_SENDER` | 미설정 | 비면 manual 모드. 발송사 `ppurio`(뿌리오)·`aligo` 지원. 뿌리오는 연동 IP 등록 필수 |
