...
브라우저에서 `http://localhost:8003` (WSL이면 Windows 크롬에서도 같은 주소). WSL IP(`http://172.x.x.x:8003`)로 열면 크롬이 엑셀·CSV 다운로드를 '안전하지 않은 다운로드 링크'로 실패시키므로 쓰지 않는다. 첫 실행 때 빈 DB와 계정이 자동으로 만들어진다
