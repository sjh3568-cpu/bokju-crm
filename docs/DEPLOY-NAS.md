...
**엑셀·CSV 다운로드가 "실패 · 안전하지 않은 다운로드 링크"로 막힐 때** — 크롬은 IP 주소로 여는
HTTP 사이트에서 시작된 다운로드를 안전하지 않다고 보고 경고·차단한다(2023년 이후 정책, 앱 버그가
아니다). 당장은 크롬 다운로드 목록에서 해당 항목의 "그래도 다운로드(보관)"를 누르면 받을 수 있다.
근본 해결은 HTTPS 주소로 여는 것이다:

1. DSM 제어판 → 로그인 포털 → 고급 → **역방향 프록시** → 생성
   - 소스: 프로토콜 HTTPS, 호스트명 `*`(또는 NAS 호스트명), 포트 `8443`
   - 대상: 프로토콜 HTTP, 호스트명 `localhost`, 포트 `8003`
2. 제어판 → 보안 → 인증서에서 이 포털에 NAS 인증서를 연결한다. Synology 기본(자체 서명)
   인증서를 쓰면 상담사 PC마다 첫 접속 때 "고급 → 이동"을 한 번 눌러야 하므로,
   가능하면 DDNS + Let's Encrypt 인증서를 만들어 붙이는 편이 깔끔하다.
3. 상담사 즐겨찾기를 `https://<NAS주소>:8443` 으로 바꾼다. 8003은 그대로 두어도 된다.
...
(2026-09-16부터 백업은 gzip 압축 저장. daily 30일 · startup 최근 5개만 자동 보관. deploy.sh의 배포 전 cp(manual_배포전)는 startup과 중복이라 없앰)
