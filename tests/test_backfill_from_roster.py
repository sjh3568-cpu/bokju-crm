...
class BackfillTests(unittest.TestCase):
    ...
    def test_apply_fills_and_is_idempotent(self):
        ins, adm = bf.run(self.conn, apply=True, quiet=True)
        self.assertEqual(ins["stats"]["채움"], 1)
        ...
        ins2, adm2 = bf.run(self.conn, apply=True, quiet=True)
        self.assertEqual(ins2["stats"]["이미 같음"], 1)
