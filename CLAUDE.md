...
- ❌ 양식에 없는 임의 워크플로 — 사용자 명시적 요청 외 추가 금지
  - **단**, 입원 진행 단계 4분류(상담완료/입원보류/입원취소/입원완료)는 통계 KPI 산출용으로 2026-05-05 명시적 요청에 따라 추가됨. `admission_status` 컬럼 사용. 2026-05-21 사용자 요청으로 상담일지 **폼 '상담 결과' 섹션**에서도 선택 가능(입원보류·입원취소는 사유 필수). 상담 상세 페이지·상담목록 인라인에서도 사후 변경 가능. 입원완료 후 단계는 `퇴원완료`(저장)·`퇴원예정`(파생 표시)이 있고, 상담목록에서 입원기간 만료 1달 전부터 퇴원예정 알림 → 퇴원완료/입원연장 처리
