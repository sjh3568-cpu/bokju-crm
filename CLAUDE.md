...
- 값이 엇갈리면 **원무 명부가 이긴다** — 단, **입·퇴원일·병실·주치의 같은 '사실 값'에 한해서다.**
  명부에 없는 것(상담사·나이·유입경로)만 상담에서 가져온다. 명부에 아직 없는 건은 숨기지 말고
  '명부 전' 표식으로 보여준다.
- ⚠ **원무 명부는 그날의 참고 자료일 뿐이다**(2026-09-25 사용자 결정). "지금 누가 재원인가"를 명부와
  매번 대조해 정하지 않는다 — 명부 회차가 열려 있어도 앱이 퇴원·외진을 알면 재원이 아니다.
  재원 판정식은 `models.crm_discharge_sql` **한 곳**(앱 회차 퇴원일 → 상담 퇴원완료일 → 미복귀 외진 출발일)이고
  재원 명단·병동 가동률·병실 만실·재원 추이·30일 census·월간보고서가 전부 이 식을 쓴다. 다른 데서 따로 세지 말 것.
- **하루치 명부에 없는 사람은 퇴원자**(2026-09-26 사용자 결정). 파일명이 `(YYYYMMDD-YYYYMMDD)` 시작=끝인 명부는
  그날 재원 전원의 스냅샷이다. 적재기(`tools/import_admission_roster.plan_absent_discharges`)가 파일에 없는 열린 명부
  회차를 닫는다 — CRM이 아는 날(상담 퇴원완료·외진 출발)이 있으면 그 날, 없으면 명부 날짜에 '추정'. 스냅샷 뒤 입원은
  건드리지 않고, 행 수가 열린 회차의 90% 미만이면 부분 파일로 보고 건너뛴다(예전에 이 사고를 겪어 아예 안 닫았었다).
  이어지는 백필 `--snapshot`이 상담도 퇴원완료로 맞춘다(날짜 모르면 미상). **CRM 회차(roster_key 없음)는 기동 때 상담값으로
  덮이므로 닫으려면 회차가 아니라 상담을 바꿔야 한다.** 지금 재원 중인 환자의 CRM 중복 회차(254건)는 이 규칙 대상이 아니다.
...
- **외진(응급전원·모병원 외래치료) 복귀는 그날의 입원으로 센다.** 원무 명부는 보통 나간 날 퇴원 회차를 닫고 복귀한 날 새 회차를 여니 명부만으로도 잡히지만, 명부에 복귀일 회차가 없으면 `admission_events.returned_at`(outcome 복귀)로 보충한다 — `models._AWAY_RETURN_NOT_IN_ROSTER`(같은 환자의 명부 회차가 복귀일에 시작하지 않는 건만) 한 조건으로 `models.admission_flow_events`가 **외진 복귀를 IN 행(source '외진', `is_return`)** 으로 넣고, 그 함수를 쓰는 세 화면이 같이 받는다: 입원·퇴원 이력 탭(`ward_moves._load`, '입원 복귀' 배지·엑셀 '입원(복귀)'), 대시보드 이번주/이번달 입원(`models.admission_flow_counts`), 명부 입원 KPI·30일 추이(`dashboard_metrics.admission_flow_by_date`). 복귀를 따로 덧붙이는 코드를 두지 말 것(2026-09-20에 대시보드만 빠진 사고). 타 병원 전원으로 끝난 건은 입원이 아니다.
