...
INSURANCE_TYPES = [
    "건강보험",
    "의료급여",   # 원무 명부 '환자유형' 값 — 1종/2종을 구분하지 않는다
    "보호1종",
    "보호2종",
    "차상위 1종",
    "차상위 2종",
...
# 병동별 허가 병상 수 — 대시보드 '병동별 재원' 가동률의 분모. 비워 두면 인원만 표시한다.
# 전체 허가 병상(355)은 app.WARD_BED_CAPACITY. 예: {"2병동": 52, "3병동": 68, ...}
WARD_BED_CAPACITIES: dict[str, int] = {
    "2병동": 59, "3병동": 75, "5병동": 37, "6병동": 40,
    "9병동": 46, "10병동": 44, "12병동": 27, "13병동": 27,
}   # 합계 355 = app.WARD_BED_CAPACITY
# 병실별 병상 수 — 대시보드 '병동별 재원'의 남/여 빈 병상 계산에 쓴다.
# 병실은 성별로 나뉘므로, 남자만 있는 방의 빈자리 = 남자 병상, 여자만 있는 방 = 여자 병상,
# 빈 방 = 성별 무관. 비어 있는 병동은 남/여 구분 없이 남은 병상만 표시한다.
# 예: {"201호": 6, "202호": 6, "204호": 4, ...}  (원무 명부 표기와 같은 '201호' 형식)
ROOM_BED_CAPACITIES: dict[str, int] = {
    # 2026-09-14 원무 확인 기준. 기본 4인실, 예외만 따로 적는다:
    #   5인실 205·206·207·303·304·305 / 2인실 306·307·903·904·905 / 1인실 502 / 3인실 1207·1307
    # 병동별 합이 WARD_BED_CAPACITIES와 같아야 화면에 '미확인'이 안 뜬다(tests/test_dashboard_strip.py가 검사).
    # 2병동 210~212호, 3병동 308호는 없는 방. 9병동은 명부에 아직 안 올라와 호실 번호를
    # 901·902·906~913으로 가정했다(4인실 10개) — 명부가 들어오면 번호만 맞출 것.
    **{f"{n}호": 4 for n in (201, 202, 203, 204, 208, 209, 213, 214, 215, 216, 217)},
    "205호": 5, "206호": 5, "207호": 5,
    **{f"{n}호": 4 for n in (301, 302, 309, 310, 311, 312, 313, 314, 315, 316, 317, 318, 319, 320)},
    "303호": 5, "304호": 5, "305호": 5, "306호": 2, "307호": 2,
    **{f"{n}호": 4 for n in (501, 503, 504, 505, 506, 507, 508, 509, 510)},
    "502호": 1,
    **{f"{n}호": 4 for n in range(601, 611)},
    **{f"{n}호": 4 for n in (901, 902, 906, 907, 908, 909, 910, 911, 912, 913)},
    "903호": 2, "904호": 2, "905호": 2,
    **{f"{n}호": 4 for n in range(1001, 1012)},
    **{f"{n}호": 4 for n in range(1201, 1207)},
    "1207호": 3,
    **{f"{n}호": 4 for n in range(1301, 1307)},
    "1307호": 3,
}
...
# 재원 화면 침상 카드의 주치의 축약 표기 '정기천(IM1)' — 명부 주치의 값(이름만)에 붙일 과 코드.
# ATTENDING_DOCTORS(상담 입력 선택지)와 별개로, 명부에만 나오는 의사도 여기 적는다. 2026-09-14 사용자 확인.
DOCTOR_DEPT_CODES: dict[str, str] = {
    "정기천": "IM1", "신현범": "IM2", "이성범": "RM1", "이석태": "RM2",
    "허남연": "RM3", "변현숙": "NE", "김우근": "FM",
}
...
REFERRAL_SOURCE_GROUPS = {
    "온라인": ["홈페이지", "카카오톡 채널", "카페", "검색(블로그)", "유튜브", "SNS"],
    "소개": ["지인추천", "직원소개", "기관연계"],
    "기타": ["지역민", "현수막"],
}
REFERRAL_TYPES = list(REFERRAL_SOURCE_GROUPS.keys())  # ['온라인', '소개', '기타']
# 채널 문의(communications.channel) → 상담일지 유입경로. 문의에서 '상담 등록'으로 넘어갈 때 자동 체크되어
# 채널 문의가 상담·입원으로 얼마나 이어졌는지 상담 통계의 유입경로 항목으로 바로 비교된다.
INBOUND_CHANNEL_REFERRAL = {"웹문의": "홈페이지", "카카오": "카카오톡 채널"}
# 채널 문의 내역 화면의 채널 표시명·필터 순서
INBOUND_CHANNEL_LABELS = {"웹문의": "홈페이지", "카카오": "카카오톡"}

# 직원소개 — 소개자를 재단 기관·부서·이름으로 나눠 입력(직원소개 분석에서 집계).
# 자유텍스트 한 칸(referrer_person)에 다 섞여 있던 것을 구조화한다.
STAFF_REFERRAL_ORGS = ["복주회복병원", "경도요양병원", "복주요양원"]
# 부서 — 정해진 목록에서 고르되, 목록에 없으면 직접 입력도 된다(콤보박스).
STAFF_REFERRAL_DEPTS = [
    "진료진", "간호부", "치료부", "치료실", "원무과", "대외협력부", "고객지원부",
    "진료협력팀", "사회사업팀", "관리과", "시설관리과", "기획실", "미래전략실",
    "영상검사실", "진단검사실", "인공신장실", "행정부",
]
...
# 현직 상담사 + 과거 이력 조회용 퇴직자. 2023~2025 엑셀 마이그레이션에서 나온
# 이름들을 추가 — 명단에 없으면 상담사별 통계·필터에서 그 이력이 통째로 빠진다.
# 전체 명부 — 과거 상담 데이터에 남아 있는 이름까지 포함한다.
# 상담목록의 담당자 필터·엑셀 임포트 검증이 이걸 쓴다. 여기서 이름을 빼면
# 그 사람이 담당했던 과거 상담을 필터로 못 찾게 되므로 함부로 지우지 말 것.
COUNSELORS = ["박세연", "권창영", "김미화", "신재희",
              "김성희", "박수정", "안순애", "신민혜",
              "이상철", "윤해원", "김윤미", "황은주", "권미경", "석정대"]

# 현재 근무 중인 상담자 — 새 상담 등록 폼의 추천 목록(2026-09-10 4명으로 정리).
# 폼은 직접 입력도 되는 콤보박스라, 목록에 없는 이름도 타이핑해서 저장할 수 있고
# 기존 상담을 수정할 때 담당자 이름이 지워지지도 않는다.
COUNSELORS_ACTIVE = ["박세연", "권창영", "김미화", "신재희"]
...
AUDIT_ACTION_LABELS = {
    ...
    "view_patient": "환자 조회",
    "view_consult": "상담 조회",
    "view_document": "팩스·문서 열람",
    "upload_document": "문서 올리기",
    "update_document": "문서 판독값 수정",
    "analyze_document": "문서 AI 판독",
    "link_document": "문서 환자 연결",
    "close_document": "문서 처리 완료",
